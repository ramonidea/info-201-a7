...
#get data
emission.data <- read.csv("./data/WDI_emissions_Data.csv", fileEncoding = "UTF-8-BOM")
emission.def <- read.csv("./data/WDI_emissions_Definition and Source.csv", fileEncoding = "UTF-8-BOM")
...
getFullName <- function(names){
  ...
  for(i in 1:nrow(names)){
    name<-as.character((filter(emission.def, as.character(Code) == as.character(names[[1]][i]))$Indicator.Name))
    result<-
      c(result,name)
...
#global.data <- as.data.frame(t(global.data))

getTrend <- function(country, series){
  if(country == "World"){
    data.result <- global.data[series,]
  }else{
    data.result <- filter(emission.data, Country.Code == iso.alpha(country,3))
  ...
  gg <- ggplot(data.result, aes(x = variable, y = value)) + geom_line(group=1, size=2, color = 'Dark Blue')
  return(gg)
}
...
getCountrynames <- function(){
  country.names <- 
    ...
    na.omit() %>% 
    select(Country.Code)
  country.names$code <- iso.expand(as.character(country.names$Country.Code))
  #Fix special country name
  country.names$code[country.names$code=="(^Barbuda)|(^Antigua)"] <- 'Barbuda'
  country.names$code[country.names$code=="(^China(?!:Hong Kong|:Macao))|(^Paracel Islands)"] <- "China"
  country.names$code[country.names$code=="Finland(?!:Aland)"] <-"Finland"
  country.names$code[country.names$code=="(^France)|(^Clipperton Island)"] <- "France"
  country.names$code[country.names$code=="Norway(?!:Bouvet|:Svalbard|:Jan Mayen)"] <- "Norway"
  country.names$code[country.names$code=="(^Portugal)|(^Azores)|(^Madeira Islands)"] <- "Portugal"
  country.names$code[country.names$code=="(^Spain)|(^Canary Islands)"] <- "Spain"
  country.names$code[country.names$code=="(^Saint Kitts)|(^Nevis)"] <-  "Saint Kitts"   
  country.names$code[country.names$code=="(^Saint Vincent)|(^Grenadines)"] <- "Saint Vincent"
  country.names$code[country.names$code=="(^Trinidad)|(^Tobago)"] <- "Trinidad"
...
get_year_emission <- function(year, country){
  emission.data<- 
    ...
    filter(Series.Code %in% c("EN.CO2.TRAN.ZS","EN.ATM.CO2E.SF.ZS","EN.CO2.BLDG.ZS","EN.CO2.OTHX.ZS","EN.CO2.MANF.ZS","EN.ATM.CO2E.LF.ZS","EN.ATM.CO2E.GF.ZS","EN.CO2.ETOT.ZS")) %>% 
    arrange_(iso.alpha(country,n=3))
  emission.data$Series.Code <- factor(emission.data$Series.Code, levels = emission.data$Series.Code)
...
getName <- function(names){
  ...
  for(i in 1:nrow(names)){
    name<-as.character((filter(emission.def, as.character(Code) == as.character(names[[1]][i]))$Indicator.Name))
    name <- strsplit(strsplit(name,"CO2 emissions ")[[1]][2],"\\(")[[1]][1]
    result<-
      c(result,name)
...
plot <-ggplot(data = country.data)+
  theme_light(base_size = 15, base_family = "")+
  geom_bar(aes_string(x = factor(1),y = code, fill = "Series.Code"),width = 1,position = "stack",stat = "identity", color="black")+
  ...
  scale_fill_brewer(palette = "Blues", name="Source", labels=source.labels )+
    theme(legend.position = "bottom",legend.direction="vertical", legend.text = element_text(size=12))+
...
global_emission_map <- function(year){
...
global.data <- mutate(global.data,year = cut(global.data[,year]/1000000,10))
...
gg <- ggplot(data=global.data) +
  geom_map(map=global.data,
                    ...
                    color="dark gray", fill="gray", size=0.05)+
  geom_map(map = global.data, 
  ...
  scale_fill_brewer(palette = "Blues", name="CO2 Emission kt (Millions)" ) +
   theme_dark()+
  ggtitle(paste0("World CO2 Emission in ",year))
